...
- ✅ ~~What happens when two modules on a mixed-version fleet expect different versions of the same interface?~~ Closed, scoped narrowly — **but depends on the Phase 0 gap above being fixed first.** As things stand on `develop` today, `add_feature` publishes unversioned `pyobs:interface:{name}` strings, so a version mismatch doesn't even surface as "interface not found" — it silently resolves, since nothing on the wire currently carries the version to mismatch on. The diagnostic-error fix below assumes `urn:pyobs:interface:{name}:{version}` feature strings are already being published and parsed; without that, there's nothing to scan. The scenario is real for distributed telescope networks — a control PC that's hard to reach or queued for maintenance can lag behind the rest of the fleet, so synchronized upgrades can't be assumed. Once versioned feature strings land, a version mismatch would surface as `_resolve_proxy` raising a generic "interface not found" `ValueError` — identical to the interface never having been implemented at all, which turns a version issue into a confusing support ticket. The fix stays cheap and local: the disco#info reply that failed to match is already in memory, so on failure, scan the other `urn:pyobs:interface:{name}:*` features already present and report the actual version found instead of a bare "not found":
  ...
  ```python
      ...
      # already have the full disco#info reply -- check whether this is a
      # version mismatch rather than a genuinely missing interface
      other = [f for f in features if f.startswith(f"urn:pyobs:interface:{obj_type.__name__}:")]
      if other:
          remote_version = other[0].rsplit(":", 1)[-1]
          direction = "upgrade the remote module" if int(remote_version) < obj_type.version else "upgrade this client"
          log.warning(
              f'"{name}" implements {obj_type.__name__} at v{remote_version}, this client expects '
              f"v{obj_type.version} ({direction})."
          )
          raise ValueError(f'"{name}" implements {obj_type.__name__} at v{remote_version}, expected v{obj_type.version}.')
      raise ValueError(f'"{name}" does not implement {obj_type.__name__}.')
  ```

  Zero extra network traffic, no new `Comm`/`Proxy` abstraction — purely a better message on an existing failure path, consistent with keeping the public surface small. Two small refinements worth including: **direction matters** ("this client expects v1, remote has v2" needs a different fix than the reverse — the message should say which machine to go patch, not just that there's a mismatch), and **log, don't just raise** — if this fires inside a retry loop (e.g. the flat-fielding migration example elsewhere in this document), a bare `raise` reproduces the same warning every retry; `log.warning(...)` once per `(module, interface)` pair (same bookkeeping shape as `_state_subscriptions`) avoids drowning the log. Fleet-wide "who's running what version" visibility is a legitimate separate want, but belongs centralized in `pyobs-web-admin` as a periodic disco#info sweep across the roster, not duplicated into every module's connect logic — an operational/monitoring concern, not something `_resolve_proxy` itself should own. Whether pyobs is meant to tolerate genuinely routine mixed-version operation, or treats it as a rare transient state during a coordinated rollout, isn't stated anywhere else in this document — worth being explicit about that assumption if it ever needs revisiting, since it's what determines whether this diagnostic-message fix is sufficient or whether the admin-dashboard version is eventually justified too.

  **Implemented on `develop`.** The Phase 0 gap above is closed alongside this: `add_feature` in `xmppcomm.py` now publishes `urn:pyobs:interface:{name}:{version}`, and `_get_interfaces` parses the versioned prefix and caches the raw feature list per JID, keeping a name in the resolved interface list only if the remote's published version matches the local class's `.version` — a mismatch is excluded exactly like a genuinely-missing interface, so it reaches the same `_resolve_proxy` failure path. Rather than inlining the scan into the generic `_resolve_proxy` (which has no wire-format knowledge and is shared by `LocalComm`, which can't have a version mismatch at all), the diagnostic lives behind a small hook — `Comm._diagnose_missing_interface(client, obj_type) -> str | None`, defaulting to `None` — that `XmppComm` overrides to scan its cached features and append the hint to the existing `ValueError` message. `_get_full_client_name`/JID-keyed dicts are reused for the per-`(client, interface)` warn-once bookkeeping rather than introducing a new identity scheme. **Known consequence, called out above but worth restating:** this is a breaking wire change for interfaces specifically — a pre-fix module and a post-fix module now see each other as implementing zero pyobs interfaces (not even `IModule`), since the feature prefix changed from bare `pyobs:interface:` to `urn:pyobs:interface:...:{version}`. `pyobs-web-client`'s live feature-matching (noted above) still needs the matching update; event features (`pyobs:event:{name}`) are untouched by this change and remain unversioned on the wire.
