...
- 🔵 **Three interfaces still missing `state =` assignment**: `IFocusModel`, `IWeather`, `IAutoFocus` — all still expose their original `get_*`/tuple-returning RPC methods. See [Phase 3](#phase-3--bulk-rollout), [Appendix: get_* to State Survey](#appendix-get_-to-state-survey).
