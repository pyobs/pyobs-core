# pyobs 2.0 — Wire Protocol, State, and Access Control

Status: implemented, closed. This was the primary design document for the pyobs 2.0
rearchitecture: adding a first-class **state** concept alongside RPC/discovery/events, a
versioned wire protocol (RPC payload encoding 2.0, typed disco#info schemas), and
per-module **access control** (ACLs). All sections below are implemented and merged to
`develop` unless marked otherwise; a handful of ✅/🔵 markers are kept inline where they
carry useful detail about what shipped versus what was originally sketched. The few items
that are still genuinely open (not just historical narration) are tracked in
`specs/plans/pyobs_2_0_work_plan.md`, not here.
...
class IDataSequence(Interface):     state = DataSequenceState    # ✅ grab_sequence()/abort_sequence(), see specs/design/idatasequence.md
