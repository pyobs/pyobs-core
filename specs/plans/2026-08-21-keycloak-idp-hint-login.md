...
- [ ] `pyobs_auth/views.py` `LoginView`: the hint comes from the `?idp_hint=` query param with
...
- [ ] `settings.py` `PYOBS_AUTH` dict: add `"IDP_HINT"` and `"IDP_LABEL"`.
...
- [ ] `specs/design/shared-auth-keycloak.md`: short note (a few lines) under the existing status
...
- [ ] `specs/plans/index.md`: this plan entry (done — see entry below).

## 4. Verification

- [ ] pyobs-auth full test suite green (client + views + existing).
...
- [ ] Regression: hint unset (or empty env) → the login page renders exactly the current single
      "Log in with Keycloak" button and behavior.
