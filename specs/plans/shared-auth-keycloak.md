...
- [ ] OIDC discovery + token handling against Keycloak — authorization-code grant (+ PKCE) for
...
- [ ] Bearer-token validation: signature/issuer/audience checks against Keycloak's JWKS endpoint —
      ...
      are retired).
- [ ] DRF `authentication.BaseAuthentication` implementation (e.g. `KeycloakAuthentication`) that
      archive/robotic-backend both wire into `REST_FRAMEWORK['DEFAULT_AUTHENTICATION_CLASSES']`.
- [ ] Config surface: one shared realm URL (same across services), per-service client ID/secret
      (one Keycloak client per service, within that shared realm — see design doc).
- [ ] User-mapping helper: resolve a validated token's `sub` claim to a local Django `User`,
      shared logic both archive and robotic-backend call rather than each reimplementing the
      `sub`-lookup — see design doc's realm/user-mapping decision.
