...
def test_parse_config_simple():
    ...
    assert pytest.approx(ete(10, binning='2x2'), 0.1) == 9.22e-7


def test_parse_config_bias():
    ete = ExpTimeEval(None, {'1x1': 'exp(-0.9*(h+3.9))', '2x2': 'exp(-0.9*(h+5.9))'})
    assert ete.binnings == ['1x1', '2x2']
    assert len(ete.filters) == 0
    assert pytest.approx(ete(10, binning='1x1'), 0.1) == 3.69e-6
    assert pytest.approx(ete(10, binning='1x1', filter_name='Clear'), 0.1) == 3.69e-6
    assert pytest.approx(ete(10, binning='2x2'), 0.1) == 6.09e-7
...
def test_parse_config_filter():
    ...
    assert pytest.approx(ete(10, filter_name='Red', binning='2x2'), 0.1) == 9.22e-7
...
def test_parse_config_full():
    ...
    assert ete.filters == ['Blue', 'Red']
    assert ete.binnings == ['1x1', '2x2']
    assert pytest.approx(ete(10, binning='1x1', filter_name='Red'), 0.1) == 3.69e-6
    assert pytest.approx(ete(10, binning='1x1', filter_name='Blue'), 0.1) == 6.09e-7
    assert pytest.approx(ete(10, binning='2x2', filter_name='Red'), 0.1) == 1.50e-6
