...
**Implementation status: self-clamped.** `Comm` gained a `get_own_capabilities(interface)` method (mirroring the existing `get_own_state`/`_get_own_state` pair, with the same base-class-default-`None` plus per-backend-override shape) alongside `_get_own_capabilities`, implemented in both `LocalComm` and `XmppComm` by reading back the same `self._capabilities` dict `_set_capabilities` already writes to — no new storage, just a new read path onto data that was already being kept. The background task's `_tracking_refresh_interval` calls `self.comm.get_own_capabilities(ITrackingRate)` and computes `max(accuracy_driven_interval, capabilities.min_update_interval)` as designed, logging a warning (once per tracked target, not once per tick) if the hardware floor ends up coarser than the target's accuracy actually needs.
